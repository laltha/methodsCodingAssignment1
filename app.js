...
app.get('/todos/', checkRequestsQueries, async (request, response) => {
  ...
  FROM todo WHERE todo LIKE "%${search_q}%" AND priority LIKE "%${priority}%" AND status LIKE "%${status}%" AND category LIKE "%${category}%";  
  `
...
app.get('/todos/:todoId/', checkRequestsQueries, async (request, response) => {
  ...
  id, todo, status, priority, category, due_date AS dueDate FROM todo WHERE id=${todoId};`
...
app.post('/todos/', checkRequestsBody, async (request, response) => {
  ...
  const createUserQuery = await db.run(addTodoQuery)
